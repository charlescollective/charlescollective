...
// Function to merge default options with user-provided options
...
// Function to convert color from hex to vec4
...
// Function to simulate movement of metaballs
var simulateMovement = function simulateMovement(_ref2) {
  var metaballs = _ref2.metaballs,
      options = _ref2.options;

  // Don't move the last interactive cursor metaball
...
// Function to get the handle for metaballs uniform
...
// Function to initialize metaballs
function initMetaballs(canvas) {
  ...
  var resize = function resize() {
    var realToCSSPixels = options.useDevicePixelRatio ? window.devicePixelRatio : 1;
  ...
  // User might not have set correct canvasWidth / canvasHeight
  resize();
...
// Function to initialize metaballs with custom options for Canvas 1
function initializeMetaballsCanvas1(options) {
  const canvases = document.querySelectorAll('.gradient-canvas');
  canvases.forEach(canvas => initMetaballs(canvas, options));
}

// Debounce function to limit the frequency of function calls for Canvas 1
function debounceCanvas1(func, delay) {
  let timeoutId;
...
// Function to adjust options based on screen size for Canvas 1
function adjustOptionsForScreenSizeCanvas1() {
  const screenSize = window.innerWidth;
  let options;

  // Define different settings for different screen sizes for Canvas 1
  if (screenSize >= 999) {
    options = {
      numMetaballs: 8,
      minRadius: 120,
  ...
  // Initialize metaballs with adjusted options for Canvas 1
  initializeMetaballsCanvas1(options);
}

// Debounced function to adjust options on window resize for Canvas 1
const debouncedAdjustOptionsCanvas1 = debounceCanvas1(adjustOptionsForScreenSizeCanvas1, 250);

// Adjust options on page load and when the window is resized for Canvas 1
window.addEventListener('load', adjustOptionsForScreenSizeCanvas1);
window.addEventListener('resize', debouncedAdjustOptionsCanvas1);
